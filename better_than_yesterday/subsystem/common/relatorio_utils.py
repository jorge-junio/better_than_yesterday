...
from better_than_yesterday.subsystem.common.relatorio_html import (
    FILTRO_ROW_TEMPLATE, HEADER_DEFAULT, HEADER_PAGINACAO, TEMPLATE_DEFAULT)
from jjsystem.common import exception
from jjsystem.common.subsystem.entity import DATE_FMT, DATETIME_FMT
from jjsystem.subsystem.image.resource import QualityImage

from better_than_yesterday.subsystem.common.relatorio_styles import (
    style_1, style_2, style_3, style_4, style_5, style_6, style_default)
from typing import List, Dict


# constantes
JJHTML2PDF_URL = os.getenv(
    'JJHTML2PDF_URL', 'http://172.17.0.1:1234/jjhtml2pdf')


# funções
def gerar_pdf(html, parametros=None, url=JJHTML2PDF_URL):
    data = {
            'html': html,
            'parametros': (
    ...
    if r.status_code != 200:
        raise exception.JJSystemException(r.text)
    else:
        return r.content
...
def get_valor_por_extenso(valor, precisao=2):
    valor_str = format_dinheiro(valor, precisao)
    valor_por_extenso = number_to_long_number(str(valor_str))
...
def get_logo_path(manager, domain_id):
    IMAGEM_DEFAULT = (
        os.environ.get('JJHTML2PDF_FILE_DIR', '') +
        '/default' + '/default-domain-image.png')
    PATH_BASE = os.environ.get('PATH_BASE_IMAGES', '/all_files/better_than_yesterday')
